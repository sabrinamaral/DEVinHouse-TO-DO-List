...
todoList.addEventListener("click", function(event) {
    if(event.target.type === "checkbox") {      // if the event target is a checkbox input
        toggle(event.target.parentElement.getAttribute('data-key'));    // call toggle function with the data-key as argument
    }
    if (event.target.classList.contains("btn-delete")) {        // if the event target is the delete button 
...
function addTodo(item) {
    if(item != "") {            // if imput is not empty, create an object
        const todoObj = {
            id: Date.now(),     // Date.now is ised to generate a random id   
            name: item,
...
function createLine(itemsArr) {
    ...
    alert.innerHTML = "Não há itens na lista";          // show alert message in the screem
...
function toggle(id) {
    itemsArr.forEach(function(item) { // it will run the array to check the conditional below
        if(item.id == id) {           // if the item id that's being clicked is the same in the array
...
function deleteTodo(id) {
    itemsArr = itemsArr.filter(function(item) { // filter the array
...
function getFromLocalStorage(){
    ...
    if(reference) { //if there is any task 
